...
calculate_beta <- function(a, dt, date){
  d <- dt[Dates < date, cov(Return, Market_Return)/var(Market_Return),
          by = Company]
...
# Cumulative returns
portfolio_return[, Cum_Return := cumprod(1+V1), by = Portfolio]

# Check if there are patterns in returns
portfolio_return[, mean(V1), by = Portfolio]

ggplot(portfolio_return, aes(Dates, Cum_Return, color = as.factor(Portfolio))) + geom_line()

ggplot(portfolio_return, aes()) + geom_point()
```
...
```{r}
portfolio_return[, Beta := cov(V1, market_return3[Dates >=
